...
import { ArrowUp, ArrowDown, Loader2, Bitcoin, Lightbulb, TrendingUp, TrendingDown } from "lucide-react"
import { PredictionHistory } from "@/components/prediction-history"

interface BitcoinFeedProps {
  detailed?: boolean
}
...
export function BitcoinFeed({ detailed = false }: BitcoinFeedProps) {
  ...
  useEffect(() => {
    const getBitcoinData = async () => {
      try {
        ...
        setError("Using simulated Bitcoin data")
    ...
    const intervalId = setInterval(getBitcoinData, 5 * 60 * 1000)
  ...
  useEffect(() => {
    ...
    const microUpdateInterval = setInterval(() => {
      if (!isLoading && !isPredicting) {
        ...
        // Create a small random change - Bitcoin is more volatile
        const volatility = 0.0008 // 0.08% volatility for micro-updates
        const randomChange = (Math.random() * 2 - 1) * volatility
        ...
        if (predictionValue !== null) {
          const predictionChange = predictionValue * (Math.random() * 0.001 - 0.0004)
          const newPredictionValue = predictionValue + predictionChange
          setPredictionValue(newPredictionValue)

          // Update confidence slightly
          setConfidenceScore(Math.min(90, Math.max(60, confidenceScore + (Math.random() * 0.5 - 0.25))))
        }

        setLastUpdated(new Date())
      }
    }, 2000) // Update every 2 seconds (Bitcoin is more volatile)
  ...
  const generatePrediction = async (marketData: any[]) => {
    try {
      ...
      setConfidenceScore(65 + Math.random() * 25) // 65-90% confidence
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 bg-black/30 border border-[#00DC82]/40 rounded-lg shadow-lg shadow-[#00DC82]/10">
        <div className="relative w-16 h-16">
          <div className="absolute inset-0 bg-[#F7931A]/20 rounded-full animate-ping"></div>
          <div className="absolute inset-2 bg-[#F7931A]/40 rounded-full animate-pulse"></div>
          <Loader2 className="absolute inset-0 m-auto h-8 w-8 animate-spin text-[#F7931A]" />
        </div>
        <p className="mt-4 text-white/70">Loading Bitcoin data...</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4 p-4 bg-black/30 border border-[#00DC82]/40 rounded-lg shadow-lg shadow-[#00DC82]/10">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold flex items-center gap-2">
            ...
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#F7931A] to-[#FFAE34]">Bitcoin</span>
          </h3>
        ...
        {isPredicting && (
          <div className="absolute inset-0 flex items-center justify-center z-10">
            <div className="bg-black/70 p-3 rounded-lg flex items-center">
              <Loader2 className="h-4 w-4 animate-spin text-[#F7931A] mr-2" />
        ...
        <ResponsiveContainer width="100%" height="100%" minWidth={280}>
          <LineChart data={chartData}>
            <defs>
              ...
              <linearGradient id="colorBitcoinPrediction" x1="0" y1="0" x2="0" y2="1">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-1 text-xs text-white/70">
                  <span>Based on time series analysis with {Math.round(confidenceScore)}% confidence</span>
                </div>
              </div>
      ...
      {detailed && (
        <div className="mt-8">
          <h3 className="text-xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-[#F7931A] to-[#FFAE34]">
            Bitcoin Market Metrics
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { metric: "Market Cap", value: "$1.23T", change: 1.8 },
              { metric: "24h Volume", value: "$42.5B", change: 3.2 },
              { metric: "Dominance", value: "51.2%", change: -0.3 },
              { metric: "Circulating Supply", value: "19.5M BTC", change: 0.01 },
            ].map((metric) => (
              <Card
                key={metric.metric}
              ...
              >
                ...
                <CardContent className="p-4 relative">
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="font-bold">{metric.metric}</div>
                    </div>
                    <div className="text-right">
